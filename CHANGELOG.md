...
## 4.4.0 — Provider-Neutral Resumable External-Agent Architecture

### Added
- `Objective_dev.md` as the maintainer/owner-intent reference for developing Project Template itself, with MUST/SHOULD/current-design constraints and explicit separation from user-project runtime scope.
- Canonical provider-neutral `EXECUTE/external_agent/` role prompts for Planning, Diagnosis, Recovery, and Evaluation.
- `scripts/agent_work.py`, `EXECUTE/control/AGENT_WORK.json`, and compact `EXECUTE/work/WORK_NNNN_RESUME.md` capsules for durable expensive-reasoning checkpoints across sessions, models, providers, and machines.
- Authoritative `STATE.json.active_external_work` pointer/sequence plus stale-binding and optimistic checkpoint-sequence protection.
- Stage-finalization gates requiring the corresponding External Agent role work item to be completed before PLAN_READY, Diagnosis registration, Recovery verification, or Evaluation finalization.
- README `VERSION / CHANGE CYCLE` as a mandatory architecture invariant and machine-validation target.

### Changed
- Workflow identity changed from Codex-centric to provider-neutral External Agent roles. Codex, Claude Code, OpenCode, Antigravity, and future repository agents may interchangeably perform expensive reasoning stages.
- Manager/Builder execution architecture remains unchanged and isolated from External Agent scratch/checkpoint context.
- README now references `Objective_dev.md` and explicitly distinguishes Project Template development intent from user-project scope.
- Completion Report -> External Research -> next Change Cycle is documented as the canonical validated-truth feedback loop.
- `STATE.json` schema advanced to v2 and workflow version to 4.4.0.

### Compatibility
- `EXECUTE/codex/*` remains as deprecated STOP/redirect stubs for one compatibility period; canonical prompts are under `EXECUTE/external_agent/`.
- External Research still has no required `Research_Vx.md` handoff.
