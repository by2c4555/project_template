...
Use this artifact only when External Agent Diagnosis has proven that a **material product/scope decision** cannot be resolved from the active immutable Scope Snapshot or repository truth.
...
When the updated handoff is `READY_FOR_PLANNING`, the user captures it with:
