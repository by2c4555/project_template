...
def scenario():
    with tempfile.TemporaryDirectory() as td:
        ...
        # Planning entry revalidates ingest and issues generation ticket.
        r=run(root,'Workplan/scripts/tools/external.py','acquire','--tool','test','--model','strong'); assert 'role: PLANNING' in r.stdout and 'generation: 1' in r.stdout
        run(root,'Workplan/scripts/tools/work.py','checkpoint','--generation','1','--unit','ARCH','--next-unit','TASKS','--note','architecture fixed')
        # Provider handoff fences old generation.
        r=run(root,'Workplan/scripts/tools/external.py','acquire','--tool','other','--model','strong2'); assert 'generation: 2' in r.stdout
        ...
        r=run(root,'Workplan/scripts/tools/planning.py','mark-ready'); assert 'NEXT_AGENT: @ExecutionManager' in r.stdout
        ...
        # Deterministic execution routing and dispatch: no user Task ID.
        ...
        # Evaluation independent Work and exact Completion binding.
        run(root,'Workplan/scripts/tools/external.py','acquire','--tool','test','--model','strong')
        ...
        # Invalid traversal declaration is rejected in a fresh reset copy.
