...
''')
        ...
        r=run(root,'Workplan/scripts/tools/execution.py','repair','--reason','Manager diagnosis: correct the bounded Task implementation without expanding authority'); st=json.loads((root/'Workplan/control/STATE.json').read_text()); wid2=st['active_work']; meta2=json.loads((root/'Workplan/work'/wid2/'WORK.json').read_text()); assert meta2['attempt_id']!=attempt1 and meta2['parent_attempt_id']==attempt1 and meta2['attempt_kind']=='REPAIR' and meta2['repair_objective'].startswith('Manager diagnosis:')
