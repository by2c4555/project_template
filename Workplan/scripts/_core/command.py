...
from .approval import create_pending, grant_matches, consume_grant
...
def execute(command, surface, tool='unknown', model='unknown'):
    # v5.3 authority tokens are literal. No trim/case normalization.
    st = load_state()
    if command not in PUBLIC_COMMANDS: return _reject(command, st, 'UNKNOWN_COMMAND')
    if surface not in {'EXTERNAL_AI','VS_CODE'}: return _reject(command, st, 'UNKNOWN_SURFACE')
    if command == 'WORKPLAN_STATUS':
        return {'status':'OK','command':command,'state_changed':False,'state':{'stage':st.get('lifecycle_stage'),'project_state':st.get('project_state'),'cycle':st.get('active_cycle'),'active_phase':st.get('active_phase'),'active_task':st.get('active_task'),'active_attempt':st.get('active_attempt'),'active_work':st.get('active_work')},'continuation':continuation(st),'allowed_commands':allowed_commands(st)}
    ...
    if command == 'EXECUTE_IMPLEMENTATION':
        ...
        return {'status':'ACCEPTED','command':command,'surface':'VS_CODE','machine_entry':'python Workplan/scripts/tools/execution.py next','protocol':'Run only the deterministic action returned. Workplan selects Phase, Task, Attempt, repair and recovery authority.','state_changed':False}
    ...
    if command in RESET_ROLE:
        ...
        subject = _subject(st, role)
        if not grant_matches(st, command, subject): return _approval(command, st, 'SUBSTANTIAL_INVALIDATION', subject)
        invalidated = _invalidate_active_work(st, role); st['next_action'] = ROLE_COMMAND[role]; consume_grant(st, command, subject)
