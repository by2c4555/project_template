...
def create_pending(risk, action, subject=''):
    ...
    aid=next_id(st,'approval','APPROVAL_')
    challenge=_challenge()
    target_seq=int(st['state_seq'])+1
    ...
    save_state(st,event='APPROVAL_CHALLENGE_ISSUED',actor='machine:risk_gate',details={'approval_id':aid,'risk_kind':risk,'requested_action':action,'generation':1})
...
def submit(challenge):
    st,rec=get_pending()
    # binding uses current exact state before submission
    ...
    if rec.get('binding_digest')!=expected or int(rec.get('bound_state_seq',-1))!=int(st['state_seq']):
        rec['status']='STALE'; rec['stale_at']=now(); atomic_write_json(APPROVAL_DIR/f"{rec['approval_id']}.json",rec)
        st['pending_approval']=None; save_state(st,event='APPROVAL_STALE',actor='human:approve',details={'approval_id':rec['approval_id']})
        return 'STALE',rec,None
    if str(challenge)!=str(rec.get('challenge')):
        rec['attempts']=int(rec.get('attempts',0))+1; rec['challenge_generation']=int(rec.get('challenge_generation',1))+1; rec['challenge']=_challenge()
        target_seq=int(st['state_seq'])+1
        projected=dict(st); projected['state_seq']=target_seq
        rec['bound_state_seq']=target_seq; rec['binding_digest']=_digest_binding(projected,rec['risk_kind'],rec['requested_action'],rec.get('subject',''))
        st['pending_approval']=rec
        save_state(st,event='APPROVAL_CHALLENGE_ROTATED',actor='human:approve',details={'approval_id':rec['approval_id'],'generation':rec['challenge_generation'],'reason':'CHALLENGE_MISMATCH'})
        return 'REJECTED',rec,rec['challenge']
    ...
    aid=rec['approval_id']; action=rec['requested_action']; st['pending_approval']=None
    st['last_granted_approval']={'approval_id':aid,'requested_action':action,'subject':rec.get('subject',''),'granted_at':rec['granted_at']}
    save_state(st,event='APPROVAL_GRANTED',actor='human:approve',details={'approval_id':aid,'requested_action':action})
    return 'GRANTED',rec,None
