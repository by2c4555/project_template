...
def _expired(rec):
    ...
    return datetime.now(timezone.utc) >= datetime.fromisoformat(raw.replace('Z','+00:00'))
...
def create_pending(risk, action, subject='', ttl_seconds=DEFAULT_TTL_SECONDS):
    ...
    rec={'approval_id':aid,'status':'PENDING','risk_kind':risk,'requested_action':action,'subject':subject,'challenge':challenge,'challenge_generation':1,'attempts':0,'bound_state_seq':target_seq,'binding_digest':_digest_binding(projected,risk,action,subject),'created_at':now(),'expires_at':_expiry(ttl_seconds)}
    st['pending_approval']=rec; save_state(st,event='APPROVAL_CHALLENGE_ISSUED',actor='machine:risk_gate',details={'approval_id':aid,'risk_kind':risk,'requested_action':action,'generation':1,'expires_at':rec['expires_at']})
...
def submit(challenge):
    st,rec=get_pending()
    if _expired(rec):
        rec['status']='EXPIRED'; rec['expired_at']=now(); atomic_write_json(APPROVAL_DIR/f"{rec['approval_id']}.json",rec); st['pending_approval']=None; save_state(st,event='APPROVAL_EXPIRED',actor='human:approve',details={'approval_id':rec['approval_id']}); return 'EXPIRED',rec,None
