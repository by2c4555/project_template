...
def project_resume():
    ...
    if pa:
        out.update({'resume_status':'HUMAN_APPROVAL_REQUIRED','approval_id':pa['approval_id'],'challenge':pa['challenge'],'risk_kind':pa['risk_kind'],'requested_action':pa['requested_action'],'human_approval_required':True}); return out
    ...
    else: out['next_action']=st.get('next_action') or 'FOLLOW_STAGE_PROTOCOL'
    return out
