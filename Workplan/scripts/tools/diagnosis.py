...
CLASSES=['IMPLEMENTATION_DEFECT','TASK_DEFECT','PLAN_DEFECT','EVALUATION_DEFECT','SCOPE_DEFECT','SCOPE_AMBIGUITY','ENVIRONMENT_DEFECT','TOOLING_DEFECT','VERIFICATION_DEFECT','EXTERNAL_BLOCKER','UNKNOWN']
p=argparse.ArgumentParser(); sp=p.add_subparsers(dest='cmd',required=True); sp.add_parser('status'); r=sp.add_parser('register'); r.add_argument('--classification',required=True,choices=CLASSES); r.add_argument('--diagnosis',required=True); a=p.parse_args(); st=load_state(); cid=st.get('active_cycle'); cycle=st['cycles'][cid]
