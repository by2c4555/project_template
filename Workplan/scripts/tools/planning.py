...
if a.cmd=='status': print('PLANNING_STATE'); print('cycle:',cid); print('stage:',st.get('lifecycle_stage')); print('ingest_verified:',bool(cycle.get('ingest'))); print('planning:',(cycle.get('planning') or {}).get('status')); raise SystemExit
...
cycle['ingest']=archived; cycle['planning']={'status':'PLAN_READY','version':'Planning_V1','revision_label':'Revision_1','based_on_scope_revision':scope['revision_label'],'based_on_scope_digest':scope['digest'],'based_on_ingest_digest':archived['package_digest'],'candidate_package_digest':manifest['package_digest'],'task_count':manifest['task_count'],'manifest':manifest}
st['cycles'][cid]=cycle; st['project_state']='PLAN_READY'; st['lifecycle_stage']='PLAN_READY'; st['next_action']='NEXT_SURFACE_VSCODE_EXECUTION_MANAGER'; save_state(st,event='PLAN_READY',actor='tool:planning',details={'package_digest':manifest['package_digest'],'task_count':manifest['task_count'],'archive_source':archived['source_root']}); mark_accepted(archived); normalize_active_ingest(); print('PLANNING: PLAN_READY'); print('package_digest:',manifest['package_digest']); print('task_count:',manifest['task_count']); print('NEXT_SURFACE: VS_CODE'); print('NEXT_AGENT: @ExecutionManager'); print('NEXT_MESSAGE: Continue Workplan.')
