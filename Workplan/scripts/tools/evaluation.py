...
if a.result=='DIAGNOSIS_REQUIRED':
    iid=next_id(st,'issue','ISSUE_'); cycle.setdefault('issues',[]).append({'issue_id':iid,'origin_type':'EVALUATION','reason':'blocking evaluation finding','status':'AWAITING_DIAGNOSIS','classification':None,'created_at':now()}); st['lifecycle_stage']='DIAGNOSIS'; st['project_state']='DIAGNOSIS'; st['next_action']='RUN_EXTERNAL_AGENT_DIAGNOSIS'; rec['issue_id']=iid
else:
    ...
    rec['completion_report']=a.completion_report; rec['completion_digest']=sha256_file(cp); cycle['status']='CLOSED_VALIDATED'; st['project_state']='CLOSED_VALIDATED'; st['lifecycle_stage']='CLOSED_VALIDATED'; st['next_action']='START_NEW_SCOPE_FOR_FUTURE_CHANGE'
st['cycles'][cid]=cycle; save_state(st,event='EVALUATION_FINALIZED',actor='tool:evaluation',details={'result':a.result,'evaluation':rec['version']}); print('EVALUATION:',a.result); print('next_stage:',st['lifecycle_stage'])
