---
name: Builder500K
description: Execute one bounded task under the 500K Builder context profile using the shared Builder Task Execution skill.
...
```text
Context ceiling:          500K
Preferred working set:   <= 300K
Maximum planned task:    <= 375K
Required reserve:         >= 125K
```
...
If planned context exceeds 384000 tokens or is `SPLIT_REQUIRED`, do not execute it as one Task under this profile.
