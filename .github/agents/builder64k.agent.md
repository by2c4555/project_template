---
name: Builder64K
description: Execute one bounded task under the 64K Builder context profile using the shared Builder Task Execution skill.
...
```text
Context ceiling:          64K
Preferred working set:   <= 40K
Maximum planned task:    <= 48K
Required reserve:         >= 16K
```
...
If planned context exceeds 49152 tokens or is `SPLIT_REQUIRED`, do not execute it as one Task under this profile.
