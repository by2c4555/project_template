---
name: Builder32K
description: Execute one bounded task under the 32K Builder context profile using the shared Builder Task Execution skill.
...
```text
Context ceiling:          32K
Preferred working set:   <= 20K
Maximum planned task:    <= 24K
Required reserve:         >= 8K
```
...
If planned context exceeds 24576 tokens or is `SPLIT_REQUIRED`, do not execute it as one Task under this profile.
