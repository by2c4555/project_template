#!/usr/bin/env python3
"""Validate a Codex Evaluation artifact and perform the authoritative PASS/blocked transition."""
...
state=load_state(); cid,cycle=active_cycle(state); ex=cycle.get('execution') or {}; ev=cycle.get('evaluation') or {}; active=ev.get('active') or {}
...
if result in {'PASS','PASS_WITH_FINDINGS'}:
    ...
    cycle['issues'].append(issue); cycle['active_issue']=issue_id; ex['active_issue']=issue_id; ex['status']='PAUSED_FOR_DIAGNOSIS'; ex['recovery']={'status':'DIAGNOSIS_REQUIRED','diagnosis':None,'resolution':None,'verification':None,'resume_authorized':False,'next_task':None,'recovery_baseline':None}; ev['status']='DIAGNOSIS_REQUIRED'; ev['next_route']='CODEX_DIAGNOSIS'; cycle['status']='RECOVERY'; cycle['lifecycle_stage']='DIAGNOSIS'; cycle['next_action']='RUN_CODEX_ISSUE_DIAGNOSIS'; state['project_state']='RECOVERY'
    ip=ROOT/'EXECUTE/issues'/f'{issue_id}.md'; ip.write_text(f'''---\nissue_id: {issue_id}\nstatus: OPEN\norigin_type: EVALUATION\norigin_evaluation: {a.evaluation}\norigin_execution: {ex.get('version')}\nopened_at: {issue['opened_at']}\nresume_authorized: false\n---\n\n# {issue_id} — Blocking Evaluation findings\n\n## Source Evaluation\n\n- `{active['report']}`\n- blocking findings: {blocking}\n\n## Next Action\n\nRun `EXECUTE/codex/ISSUE_DIAGNOSIS_PROMPT.md`. Do not modify production code during diagnosis.\n''',encoding='utf-8')
