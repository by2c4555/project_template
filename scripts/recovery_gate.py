...
from workflow_state import ROOT, active_cycle, append_transition, ensure_package_integrity, load_state, save_state, utc_now, require_external_work_complete
...
a=ap.parse_args()
state=load_state(); cid,cycle=active_cycle(state); require_external_work_complete(state,cycle,'RECOVERY'); ex=cycle.get('execution') or {}
