...
cycle['lifecycle_stage']='RECOVERY'; state['active_external_work']=None; cycle['next_action']='RUN_EXTERNAL_AGENT_RECOVERY'; save_state(state); append_transition('RECOVERY_APPROVED',actor='user:approve_recovery',cycle_id=cid,details={'issue':issue_id,'recovery_approval':rid,'diagnosis_digest':digest})
print('RECOVERY_APPROVAL: PASS'); print(f'recovery_approval_id: {rid}'); print('Next: run an External Agent with EXECUTE/external_agent/RECOVERY_PROMPT.md. Recovery must stop at VERIFIED; it cannot authorize resume.')
