#!/usr/bin/env python3
"""HUMAN/OPERATOR-ONLY authorization to start one bounded independent External Agent Evaluation attempt."""
...
ev['active']=record; ev['status']='AUTHORIZED'; ev['next_route']='RUN_EXTERNAL_AGENT_EVALUATION'; cycle['evaluation']=ev; cycle['status']='EVALUATION'; cycle['lifecycle_stage']='EVALUATION'; cycle['next_action']='RUN_EXTERNAL_AGENT_EVALUATION'; state['project_state']='EVALUATION'; state['active_external_work']=None
...
print('START_EVALUATION: PASS'); print(f'evaluation_version: {version}'); print(f'expected_report: EXECUTE/evaluation/{version}.md'); print('Next: run an External Agent with EXECUTE/external_agent/EVALUATION_PROMPT.md. One authorization covers one evaluation attempt only.')
