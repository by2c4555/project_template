#!/usr/bin/env python3
"""Shared machine-state and integrity helpers for Project Template v4.4.0.
...
def validate_project_details_handoff() -> Dict[str, Any]:
    ...
    if markdown_value(text, "artifact_status") != "READY_FOR_PLANNING":
        errors.append("project_details artifact_status must be READY_FOR_PLANNING")
...
def require_external_work_complete(state: Dict[str, Any], cycle: Dict[str, Any], role: str) -> Dict[str, Any]:
    """Require the active provider-neutral External Agent work item to be completed for this role/cycle."""
    ref = state.get("active_external_work") or {}
    if ref.get("cycle_id") != cycle.get("cycle_id") or ref.get("role") != role or ref.get("status") != "COMPLETED":
        raise SystemExit(
            f"EXTERNAL_WORK: BLOCKED\n{role} work must be marked COMPLETED with scripts/agent_work.py before this stage can finalize"
        )
    path = ROOT / str(ref.get("work_path") or "")
    if not path.is_file():
        raise SystemExit("EXTERNAL_WORK: BLOCKED\nactive external work artifact is missing")
    try:
        work = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise SystemExit(f"EXTERNAL_WORK: BLOCKED\ninvalid external work artifact: {exc}")
    if work.get("work_id") != ref.get("work_id") or work.get("checkpoint_seq") != ref.get("checkpoint_seq"):
        raise SystemExit("EXTERNAL_WORK: BLOCKED\nSTATE.json external-work pointer does not match AGENT_WORK.json")
    if work.get("status") != "COMPLETED" or work.get("role") != role:
        raise SystemExit("EXTERNAL_WORK: BLOCKED\nexternal work artifact is not completed for the required role")
    return work
